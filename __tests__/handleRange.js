...
describe("handleRange", () => {
    ...
    describe("greedy", () => {
        test("should handle minimum quantifier", () => {
            ...
            expect(lastElement.quantifier).toEqual("atLeast3-greedy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
                `"${lastElementUnquantified.value}" repeated at least 3 times`,
            );
        ...
        test("should handle minimum and maximum quantifiers", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index);
            expect(lastElement.quantifier).toEqual("3to5-greedy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
                `"${lastElementUnquantified.value}" repeated at least 3 times and no more than 5 times`,
            );
            expect(result).toEqual({ index: index + betweenBraces.length + 1 });
        });
        test("should handle if minimum and maximum quantifiers are the same", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index);
            expect(lastElement.quantifier).toEqual("exactly3-greedy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
                `"${lastElementUnquantified.value.repeat(3)}"`,
            );
            expect(result).toEqual({ index: index + betweenBraces.length + 1 });
        });
        test("should handle if only one quantifier given", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index);
            expect(lastElement.quantifier).toEqual("exactly7-greedy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
                `"${lastElementUnquantified.value.repeat(7)}"`,
            );
            expect(result).toEqual({ index: index + betweenBraces.length + 1 });
        });
    ...
    describe("lazy", () => {
        test("should handle minimum quantifier", () => {
            ...
            const index = 5;
            const result = handleRange(lastElement, betweenBraces, index, "?");
            expect(lastElement.quantifier).toEqual("atLeast3-lazy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
            ...
            expect(result).toEqual({ index: index + betweenBraces.length + 2 });
        });
        test("should handle if no minimum quantifier given", () => {
            const lastElement = { ...lastElementUnquantified };
            const betweenBraces = ",5";
        ...
        test("should handle minimum and maximum quantifiers", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index, "?");
            expect(lastElement.quantifier).toEqual("3to5-lazy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
        ...
        test("should handle if minimum and maximum quantifiers are the same", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index, "?");
            expect(lastElement.quantifier).toEqual("exactly3-lazy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
        ...
        test("should handle if only one quantifier given", () => {
            ...
            const index = 2;
            const result = handleRange(lastElement, betweenBraces, index, "?");
            expect(lastElement.quantifier).toEqual("exactly7-lazy");
            expect(lastElement.regex).toEqual(
                `${lastElementUnquantified.regex}{${betweenBraces}}`,
            );
            expect(lastElement.value).toEqual(
